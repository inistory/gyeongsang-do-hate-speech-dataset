...
def generate_mitigated_dialect(model, tokenizer, original_dialect, masked_dialect, offensive_span, model_name, use_masked=True, max_retries=3):
...
def process_data(input_file, output_dir, model_names):
    # 데이터 로드
        ...
        for use_masked in [True, False]:
            ...
            for item in tqdm(data, desc=f"{model_name} {'마스킹 정보 있음' if use_masked else '마스킹 정보 없음'} 처리 중"):
                ...
                mitigated_dialect = generate_mitigated_dialect(
                    model, tokenizer, original_dialect, item.get('masked_dialect', ''), offensive_span, model_name, use_masked
                )
...
if __name__ == "__main__":
    ...
    output_dir = "./results"
    model_names = ["princeton-nlp/gemma-2-9b-it-SimPO", "FacebookAI/roberta-base"]
    
    # 결과 디렉토리 생성
    os.makedirs(output_dir, exist_ok=True)
    
    process_data(input_file, output_dir, model_names) 